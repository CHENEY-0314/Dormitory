package com.example.dormitory.MyPagesActivity;
...
import com.example.dormitory.BuildConfig;
...
public class ChangeUserImgActivity extends AppCompatActivity {
    ...
    private ImageView ImgUser,back;
    private Button mbtnByAlbum,mbtnByCamera;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_change_user_img);

        back=findViewById(R.id.ACU_back);
        ImgUser=findViewById(R.id.ACU_userimg);
        mbtnByAlbum=findViewById(R.id.ACU_ChangeByAlbum);
        mbtnByCamera=findViewById(R.id.ACU_ChangeByCamera);
        ...
        //点击通过照相更换头像
        mbtnByCamera.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        //点击通过相册上传更换头像
        mbtnByAlbum.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                checkReadPermission();
            }
    ...
    private void checkStoragePermission() {
        ...
        if (result == PackageManager.PERMISSION_DENIED||resultCAMERA == PackageManager.PERMISSION_DENIED) {
            ...
            choseHeadImageFromCameraCapture(); // 启动手机相机拍摄照片作为头像
        }
    }
    ...
    private void checkReadPermission() {
        ...
        if (permission==PackageManager.PERMISSION_DENIED){
            ...
            choseHeadImageFromGallery(); // 从本地相册选取图片作为头像
        }
    }
    ...
    private void choseHeadImageFromCameraCapture() {
        ...
        if (hasSdcard()) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                pictureUri = FileProvider.getUriForFile(ChangeUserImgActivity.this, BuildConfig.APPLICATION_ID+".fileProvider", pictureFile);
            } else {
                intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        ...
        switch (requestCode) {
            ...
            case CODE_CAMERA_REQUEST:
                if (hasSdcard()) {
                    ...
                    Toast toast=Toast.makeText(ChangeUserImgActivity.this,null,Toast.LENGTH_SHORT);
...
//        intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
    ...
    private void setImageToHeadView(Intent intent,Bitmap b) {
        try {
            if (intent != null) {
                final Bitmap bitmap = imageZoom(b);  //压缩图片
                ImgUser.setImageBitmap(bitmap); //更新头像
                //myDialog = ProgressDialog.show(this, "Loading...", "Please wait...", true, false);
                new Thread(new Runnable() {
                    public void run() {
                        //upload(bitmap);  //将头像上传到云端,并保存到本地
                    }
                }).start();
            }
        } catch (Exception e) {
